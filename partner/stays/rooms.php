...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_room'])) {
    ...
    if (!$stmt->fetch()) {
        ...
        if ($roomId > 0) {
            ...
            ");
            $stmt->execute([$roomName, $description, $maxGuests, $numRooms, $basePrice, 
                           $sizeSqm, $bedConfig, $roomAmenities, $isActive, $roomId, $stayId]);
            ...
            ");
            $stmt->execute([$stayId, $roomName, $description, $maxGuests, $numRooms, 
                           $basePrice, $sizeSqm, $bedConfig, $roomAmenities, $isActive]);
